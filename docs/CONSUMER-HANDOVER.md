...
- **Mind the option budget.** Each option eats tokens out of a fixed head budget (~192 english); past ~20 long options you're truncatin' descriptions, and past the renderer's ceiling the request fails instead of degradin'. If your label set is huge, split it into a coarse question then a fine one — two cheap calls beat one failed call.
